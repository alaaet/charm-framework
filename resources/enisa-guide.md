...
economy, to boost the resilience of the Union’s infrastructure and, ultimately, to keep Europe’s society and citizens
...
ACKNOWLEDGEMENTS
This publication was developed by ENISA, in collaboration with the European Commission and the Network and
Information Systems Cooperation Group. ENISA would like to thank for their efforts the Network and Information
Systems Cooperation Group, along with the ENISA European Competent Authorities for Trust Services Expert Group
and the European Competent Authorities for Secure Electronic Communications. Moreover, ENISA would like to
recognise the valuable feedback of the individuals, private organisations, associations and members of the open
source software community that contributed to the open consultation.
LEGAL NOTICE
...
purposes only and must be accessible free of charge. All references to it or its use as a whole or in part must mention
ENISA as its source.
...
```
provided appropriate credit is given and any changes are indicated. For any use or reproduction of elements that are
not owned by the European Union Agency for Cybersecurity, permission may need to be sought directly from the
...
2
EXECUTIVE SUMMARY
Under the NIS2 Directive, EU member states set requirements for cybersecurity risk management measures at national
level in critical sectors, for example digital infrastructures, energy, transport or health. However, for several subsectors
of the digital infrastructures and the ICT service management sectors, these requirements are set at European Union
level due to their cross-border nature. For this purpose, the European Commission published the Implementing
```
Regulation (EU) 2024/2690 on 17 October 2024 (1), which lays down the technical and methodological requirements of
```
```
the measures referred to in Article 21(2) of Directive (EU) 2022/2555. The technical and methodological requirements
```
...
• guidance, that is, indicative and actionable advice on parameters to consider when implementing a
```
requirement;
```
```
...
```
• mappings from security requirements to industry good practices, European and international standards, and
national frameworks.
...
To develop this technical guidance, ENISA worked closely with the European Commission and the Network and
Information Systems Cooperation Group, and ENISA held a public consultation to get input from the private sector. This
guidance is a living document because it maps the technical and methodological requirements referred to in Article 2
```
...
June 2025, version 1.0
3
and the Annex to the implementing regulation, to European and international standards and to national cybersecurity
management frameworks. Both the standards and the management frameworks are constantly subject to change.
Therefore, a review process should be initiated at regular intervals by ENISA in collaboration with the European
Commission and the Network and Information Systems Cooperation Group, for instance to add additional mappings
industry good practices, international standards and national frameworks.
DISCLAIMER
This document is not legally binding and is only of an advisory character. It is not intended to replace the
frameworks, guidance, tools or other mechanisms provided by Member States at national level.
It should be clarified that the Member States retain the freedom to determine their approach to the
supervision of the requirements under this implementing regulation. Therefore, this ENISA technical
guidance is not able to define whether an entity needs to have all or just some of the ‘evidence’ listed
```
(although requiring all the ‘evidence’ listed here would be a very strict approach to supervision). This
```
document also does not determine the appropriateness of such measures or evidence. However, it can
help the national competent authorities develop their approach to the supervision of the requirements.
Entities in scope of the regulation should check under whose jurisdiction they fall and they should follow
```
any guidance by national competent authorities (see recital 7 of the implementing regulation).
```
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
4
...
ACQUISITION, DEVELOPMENT AND MAINTENANCE 76
...
7. POLICIES AND PROCEDURES TO ASSESS THE EFFECTIVENESS OF
CYBERSECURITY RISK-MANAGEMENT MEASURES 107
8. BASIC CYBER HYGIENE PRACTICES AND SECURITY TRAINING 111
8.1 AWARENESS RAISING AND BASIC CYBER HYGIENE PRACTICES 111
8.2 SECURITY TRAINING 113
9. CRYPTOGRAPHY 117
10.HUMAN RESOURCES SECURITY 122
...
June 2025, version 1.0
6
11.4 ADMINISTRATION SYSTEMS 137
11.5 IDENTIFICATION 138
11.6 AUTHENTICATION 141
11.7 MULTI-FACTOR AUTHENTICATION 144
12.ASSET MANAGEMENT 148
12.1 ASSET CLASSIFICATION 148
12.2 HANDLING OF ASSETS 149
12.3 REMOVABLE MEDIA POLICY 151
12.4 ASSET INVENTORY 152
12.5 DEPOSIT, RETURN OR DELETION OF ASSETS UPON TERMINATION OF EMPLOYMENT 154
13.ENVIRONMENTAL AND PHYSICAL SECURITY 157
13.1 SUPPORTING UTILITIES 157
13.2 PROTECTION AGAINST PHYSICAL AND ENVIRONMENTAL THREATS 159
13.3 PERIMETER AND PHYSICAL ACCESS CONTROL 161
ANNEX I NATIONAL FRAMEWORKS 165
ANNEX II GLOSSARY 167
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
7
INTRODUCTION
BACKGROUND
On 18 October 2024, the European Commission published Commission Implementing Regulation 2024/2690 of
```
17 October 2024, hereafter the regulation, pursuant to Articles 21(5), first subparagraph and 23(11), second
```
```
subparagraph, of Directive (EU) 2022/2555 (hereafter the NIS2 Directive). Article 2 of this regulation specifies that, for
```
```
the essential and important entities in scope of the regulation (hereafter ‘relevant entities’), the technical and
```
```
methodological requirements of cybersecurity risk-management measures referred to in Article 21(2), points (a) to (j),
```
of the NIS2 Directive are set out in the Annex to the regulation.
```
According to recital 7, the European Union Agency for Cybersecurity (ENISA) can support relevant entities by
```
providing guidance on the implementation of the technical and methodological requirements referred to in the Annex
to the regulation. This implementation guidance was developed by ENISA is collaboration with the Network and
```
Information Systems Cooperation Group (NIS CG) and the European Commission. ENISA published an early draft of
```
this guidance as part of a public consultation, to solicit feedback and input from the private sector. ENISA also
consulted with various relevant workstreams of the NIS Cooperation group, as well as the ENISA expert group for
```
European Competent Authorities for Trust Services (ECATS) and the ENISA expert group for European Competent
```
```
Authorities for Secure Electronic Communications (ECASEC).
```
GOAL
The document provides non-binding guidance for relevant entities to the regulation on the technical and
methodological requirements of the cybersecurity risk management measures.
TARGET AUDIENCE
Beyond the relevant entities to the regulation, this guidance may provide indications on the technical and
methodological requirements of the cybersecurity risk management measures of the NIS2 Directive, which may be
considered useful by other public or private bodies for improving their cybersecurity.
STRUCTURE
The Annex to the regulation consists of 13 titles with a varying number of technical and methodological requirements
```
(see Figure 1). Each technical and methodological requirement is highlighted (blue font and grey background) and is
```
included in this document for readability.
```
Each requirement is followed by three elements: guidance, examples of evidence and tips (2). This part of the
```
```
document is not legally binding and is only recommendations (3).
```
1. The guidance section contains indicative and actionable advice on parameters to consider when
implementing a technical and methodological requirement or further explanation of concepts found in the
legal text.
2. Examples of evidence are indicative types of evidence showing that a technical and methodological
...
```
```
( 3 ) A recommendation is defined as an ‘expression, in the content of a document, that conveys a suggested possible choice or course of action
```
deemed to be particularly suitable without necessarily mentioning or excluding others’.
A requirement is defined as an ‘expression, in the content of a document, that conveys objectively verifiable criteria to be fulfilled and from which no
```
deviation is permitted if conformance with the document is to be claimed’ (ISO/IEC Directives, Part 2 – Principles and rules for the structure and
```
```
drafting of ISO and IEC documents, 9th edition, 2021).
```
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
8
3. In some technical and methodological requirements, extra general tips are also offered for additional
...
The guidance, examples of evidence and tips are non-exhaustive. Their partial or complete implementation does
not assume compliance or conformity with the requirements of the regulation. Relevant entities may choose
alternative methods to fulfil a requirement or use different evidence to demonstrate compliance. Moreover, a single
```
piece of evidence may support various requirements; for example, an organisational chart can demonstrate both ‘roles
```
and responsibilities’ and ‘segregation of duties’. Consequently, evidence may appear multiple times within the text.
```
Finally, each requirement is mapped to (a) requirements of European and international standards or frameworks
```
```
...
10
```
ISO/IEC 27002:2022 (4), NIST Cybersecurity Framework 2.0, ETSI EN 319 401 V3.1.1 (2024–06), European
```
```
Committee for Standardization Technical Specification (CEN/TS) 18026:2024) and (b) to the NIS2 national
```
```
frameworks (5). The mapping to standards and frameworks is available on the ENISA website in Excel format (6).
```
It should be noted that the mapping was done only to horizontal standards and for specific topics. Detailed standards
or technical specifications are referenced in footnotes, where available. These standards are included as examples,
as they are commonly used. The choice of which standards and good practices to apply should be made by the entity,
as they may relate to the context of the entity’s activities and characteristics.
To implement the requirements, the relevant entities may build upon their current usage of standards or frameworks, if
available. Annex I of this document provides details for each national framework submitted to ENISA during the
consultation phases, while some terms are explained in Annex II. The document does not aim to establish a new
```
standard or to duplicate existing ones (e.g. ISO, IEC and CEN). The guidance is written in a technology-neutral and
```
standards-neutral way.
The mapping should not be interpreted as a measure of equivalency among different standards or frameworks. It
simply refers to relevant requirements in these standards or frameworks without assessing whether these fully cover
the requirements of the regulation. Cybersecurity standards or frameworks often address the same cybersecurity
concerns but use different language, structures or levels of specificity or detail. Understanding these relationships may
help relevant entities use and integrate multiple standards or frameworks efficiently, to maintain compliance, reduce
duplication and streamline audits.
Relevant entities subject to the regulation can use national frameworks, guidance, standards or other mechanisms
equivalent to the requirements of the regulation to demonstrate their compliance to national competent authorities.
Depending on the national framework, compliance with the requirements set out by the regulation could be
demonstrated by means of assessment or certification by relevant accredited conformity assessment bodies or by
independent auditors authorised by the national competent authorities or certification bodies authorised by the
national competent authorities, against the national frameworks, guidelines, standards or other mechanisms
equivalent to technical and methodological requirements for cybersecurity risk-management measures. To keep the
current guidance up to date, Member States can inform ENISA of those equivalent national frameworks, guidance,
standards or other mechanisms, if available.
TOPIC-SPECIFIC POLICIES
```
As described in preamble 9 of the regulation, the policy on the security of network and information systems (7)
```
```
(Annex to the regulation, point 1.1) should be the highest-level document setting out the relevant entities’ overall
```
approach to the security of their network and information systems and should be approved by the management bodies
of the entities.
```
( 4 ) The information security controls listed in Table A.1 of Annex A to this standard are directly derived from and aligned with those listed in ISO/IEC
```
```
27002:2022. Clauses 5 to 8 are to be used in context with clause 6.1.3 (information security risk treatment) of ISO/IEC 27001:2022.
```
```
( 5 ) The mapping is based on the information that the representatives of the Member States in the NIS Cooperation Group work stream on security
```
measures have provided to ENISA.
```
( 6 ) ENISA Technical implementation guidance mapping table (Excel file), https://www.enisa.europa.eu/publications/nis2-technical-implementation-
```
guidance
```
( 7 ) Article 21(2), point (a) of the NIS2 Directive.
```
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
11
```
In addition to this overarching corporate policy, the following topic-specific, documented policies (8) are required.
```
...
5. Policy to assess the effectiveness of cybersecurity risk management measures (Annex to the regulation,
```
point 7.1.1).
```
...
8. Policies for the management of privileged accounts and system administration accounts (Annex to the
```
regulation, point 11.3)
```
...
Documenting the aforementioned topic-specific policies is mandatory. However, the relevant entity should use its
discretion to determine the format of documentation, whether it be through separate documents, updates to existing
policies or a single comprehensive document.
```
( 8 ) According to ISO/IEC 27002:2022, topic-specific policy includes ‘intentions and direction on a specific subject or topic, as formally expressed by the
```
...
o Depending on the context, external parties may mean suppliers, service providers, shareholders,
authorities, visitors, external interest groups or forums.
o The acknowledgement may be included in other contracts, such as employment contracts or
service provision contracts.
o The policy should be communicated to relevant personnel and interested external parties in a
form that is relevant, accessible and understandable to the intended reader.
o Relevant personnel and relevant interested external parties may not be made aware of the full
...
o For contractual requirements of direct suppliers and service providers, point 5.1.4 of the Annex
to the regulation.
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
14
...
```
o allocation of resources for policy implementation;
```
```
o requests (e.g. announcements, emails and documents) to personnel to apply network and
```
```
information security in accordance with the established policies and procedures; and
```
o any initiatives that indicate that management bodies promote improvement in the area of network
...
```
```
o updates to the risk assessment results and the risk treatment plan (Annex to the regulation,
```
```
point 2.1.4);
```
```
o relevant changes in legislation (laws, regulations and other measures imposed by national
```
```
competent authorities);
```
```
o recommendations provided by relevant authorities;
```
```
o relevant changes in industry good practices;
```
```
o feedback from interested parties;
```
```
o findings of compliance monitoring (section 2.2) and of independent reviews (Annex to the
```
```
regulation, point 2.3), including policy violations or policy exceptions;
```
o incidents, even those affecting similar entities in the sector.
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
15
...
```
```
o changes to the network and information systems;
```
```
o changes to the environment of operation;
```
```
o problems identified during the implementation of the policy;
```
```
o findings of compliance monitoring (Annex to the regulation, point 2.2.1) and of independent
```
```
reviews (Annex to the regulation, point 2.3), including policy violations or policy exceptions;
```
```
o the status of preventive and corrective actions;
```
```
o trends related to threats and vulnerabilities; and
```
o known reported security incidents.
...
```
o legislative, regulatory and contractual requirements;
```
```
o awareness and training requirements (Annex to the regulation, points 8.1 and 8.2); and
```
```
o business continuity requirements (Annex to the regulation, point 4.1).
```
...
16
```
o protected in terms of confidentiality (on a need-to-know basis), integrity, availability and
```
```
authenticity;
```
o managed properly so the information is complete, correct, understandable, easily identifiable and
retrievable.
EXAMPLES OF EVIDENCE
...
```
```
o workshop and seminar attendance; and
```
o continuous learning materials.
...
obligations.
o For contractual requirements of direct suppliers and service providers, see point 5.1.4 in the
Annex to the regulation.
o Consider third parties, which means external entities or organisations not directly involved in the
...
o The network and information security policy, topic-specific policies and procedures of the relevant
...
```
```
o significant incidents, if any;
```
o changes to the environment of operation, including changes to the network and information
```
systems;
```
...
```
```
o a risk treatment option (for example risk avoidance, risk mitigation, risk transfer or sharing or risk
```
```
acceptance);
```
...
```
```
o business strategic objectives;
```
```
...
```
```
o acceptable downtime for systems for which criticality is high (e.g. up to two hours of downtime
```
```
per month);
```
```
o tolerance for data loss (e.g. loss of data with low criticality within a 24-hour window);
```
```
o maximum financial loss that can be absorbed without jeopardizing operations (e.g. up to EUR
```
```
...
23
```
o willingness to invest a certain percentage of revenue in measures (e.g. 5% of annual revenue);
```
```
o adherence to regulatory obligations with specific penalties or fines influencing risk acceptance;
```
```
o acceptable level of customer dissatisfaction or negative media exposure from a data breach (e.g.
```
```
tolerating one major incident every few years);
```
```
o acceptance of certain vulnerabilities based on risk mitigation measures in place (e.g. outdated
```
```
software provided that it is monitored and patched regularly);
```
```
o time frame for responding to and recovering from incidents (e.g. a maximum of 48 hours for
```
```
containment); and
```
o acceptance of minor incidents as part of normal operations while prioritizing major threats.
```
• Define risk acceptance criteria, which may include (indicative, non-exhaustive list) (15):
```
o accepting risks categorized as low severity, such as minor data leaks that don't expose sensitive
```
information;
```
```
o accepting risks assessed as having a low likelihood of occurrence (e.g. certain rare types of
```
```
cyberattacks);
```
```
o accepting risks if the cost of mitigation exceeds the potential impact (e.g. not upgrading legacy
```
```
systems if the upgrade cost is significantly higher than potential losses);
```
o accepting specific compliance risks if there is a plan in place to address them within a defined
```
...
within six months);
```
o allowing certain risks in low-criticality systems or departments that do not affect core business
```
operations (e.g. accepting a risk in a test environment);
```
```
o accepting certain vulnerabilities for a defined period while planning for remediation (e.g.
```
```
accepting the risk of outdated software for three months until a full upgrade can be completed);
```
o accepting risks where the expected incident impact falls below a predetermined financial
```
threshold (e.g. losses under EUR 50,000 accepted without further action);
```
o accepting risks after informing stakeholders and receiving their agreement, particularly if they
```
understand the trade-offs involved; and
```
o accepting residual risks where existing measures reduce the likelihood or impact to an acceptable
```
...
```
o alignment with risk appetite: accepting risks that align with the organization's overall risk appetite,
```
such as delays in internal reporting processes that do not impact customer service or compliance;
```
o regulatory and legal exposure: prioritizing risks based on potential legal or regulatory
```
...
```
o reputational impact: escalating risks that could significantly harm the organization's public image,
```
such as incidents likely to attract media attention;
```
o risk velocity: giving higher priority to risks with high velocity, such as zero-day exploits that can
```
...
June 2025, version 1.0
24
o recovery complexity: treating risks as critical if they involve complex or time-consuming recovery,
```
such as cyber-attacks on legacy systems with limited vendor support;
```
o emerging technology risks: Considering risks involving emerging technologies as higher priority,
such as AI systems producing unexplainable or non-compliant decisions due to lack of
```
transparency;
```
o stakeholder sensitivity: re-evaluating risks that concern key stakeholders, such as risks flagged
...
```
```
o the importance of assets;
```
```
o the severity of threats;
```
```
o the vulnerability of network and information systems (16);
```
```
o impact analysis;
```
```
o existing measures; and
```
o stakeholders concerns or requirements.
...
o of the risk treatment options or a combination of them, in line with the results of the risk
...
```
o specific risk treatment measure.
```
...
o results of audits and previous reviews,
```
o status of implementation of the measures described in the risk treatment plan (see the policy on
```
...
```
```
o changes to the information systems;
```
```
o changes to the environment of operation;
```
```
o post-incident review findings (section 3.6); and
```
o trends and changes related to threats and vulnerabilities, as they may affect risks.
EXAMPLES OF EVIDENCE
...
```
o Risk avoidance: as a measure to treat this risk, the entity might choose to eliminate activities or
conditions that expose the entity to this risk, for example, discontinuing the use of a vulnerable
software application.
o Risk mitigation: as a measure to treat this risk, the entity might choose to implement measures
...
June 2025, version 1.0
26
o Risk transfer or sharing: as a measure to treat this risk, the entity might choose to shift parts of
...
o Risk acceptance: as a measure to treat this risk, the entity might choose to acknowledge the risk
...
```
o key metrics,
o compliance status, including policy exceptions,
...
(indicative, non-exhaustive list):
```
o software updates: if a system relies on an older version of software that is incompatible with the
latest update, an exception might be granted to delay the update until a compatible solution is
```
found;
```
```
o access control: if a particular user or system cannot support an authentication mechanism (e.g.
```
```
...
```
o encryption: if a legacy system does not support encryption, an exception might be granted until
...
```
```
o objectives and a high-level approach to compliance monitoring;
```
```
o relevant security policies that are subject to compliance monitoring;
```
```
o the frequency of compliance reviews;
```
```
o who should carry out compliance reviews (internal or external); and
```
o templates for compliance review reports.
...
```
```
o changes to the threat landscape and cybersecurity legal and regulatory requirements;
```
```
o changes to standards; and
```
o changes to the policy on the security of network and information systems and/or topic-specific
policies.
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
28
...
```
```
o cybersecurity technical knowledge, for example cybersecurity frameworks (ISO/IEC 27001,
```
```
...
o compliance and regulatory knowledge, for example the NIS2 Directive, the GDPR and the Digital
Operational Resilience Act,
o good understanding of good practices in auditing and
o good understanding of when technical certification and conformance are required and must be
...
```
```
o scope and purpose of the independent reviews (e.g. compliance, risk assessment, policy
```
```
adherence);
```
```
o methodology of the reviews (e.g. standardised checklist, standard based, ad hoc, how it
```
```
...
```
```
o who should carry out independent reviews (internal or external); and
```
...
```
```
o set up a review committee with members from different departments;
```
...
2.3.3. The results of the independent reviews, including the results from the compliance monitoring pursuant to point
2.2. and the monitoring and measurement pursuant to point 7, shall be reported to the management bodies. Corrective
...
(indicative, non-exhaustive list):
```
o executive summary, including the scope and key findings,
o methodology,
o detailed findings, including gaps identified and non-compliance issues,
...
```
```
o changes to the threat landscape and cybersecurity legal and regulatory requirements; and
```
o changes to the policy on the security of network and information systems and/or topic-specific
policies.
EXAMPLES OF EVIDENCE
...
```
o ensuring that they aim to minimise disruptions, protect assets and ensure a swift return to normal
```
operations;
```
```
o identifying interfaces between the incident handling policy and business continuity management;
```
```
o describing workflows which trigger business continuity (Annex to the regulation, points 4.1, 4.2
```
```
or 4.3) during an incident; and
```
o developing scenarios that test the interaction between these processes and describe the result
...
consequences and the priority of an incident, together with the criteria for categorising events as incidents
```
(18). An indicative, non-exhaustive list of criteria might include one or more of the following:
```
o impact on business operations,
o data sensitivity in accordance with risk management,
o legal and regulatory impact, including reporting timelines stemming from regulatory framework
e.g. GDPR, national regulations,
o scope and scale meaning the evaluation of how widespread the event is,
```
o type of attack (19),
```
o malicious software/vulnerability exploitation,
o criticality of the systems affected,
o incident containment urgency,
o potential of data exfiltration or corruption, such as in the case of ransomware,
o likelihood of recovery,
o impact on human lives and safety and
o other criteria on what constitutes a significant incident as per this regulation.
```
...
```
```
o system failures and loss of service availability;
```
```
o malicious code;
```
...
```
```
o breaches of confidentiality and integrity; and
```
o misuse of network and information systems.
...
```
o purpose and scope of the plan,
o roles and responsibilities for communication tasks,
o list of internal and external stakeholders to be informed,
o reporting timelines stemming from regulatory framework, e.g. GDPR, national regulations,
o conditions and procedures for escalation of incidents,
```
o channels to be used for communication (e.g. email, intranet, phone calls, social media, press
```
```
releases),
```
```
o channels of communication need to be tailored based on the target audience (that is internal,
```
```
client or general public, etc.),
```
o methods for stakeholders to provide feedback or ask questions and
```
...
June 2025, version 1.0
34
o guidelines for when to communicate and the frequency of updates, along with pre-drafted
...
```
o tabletop exercise,
o simulation of an incident, preferably based on a selected attack scenario based on identified risks
and the current threat landscape,
o red team/blue team exercise and
o past incident walk-through.
...
regulation:
```
o results from the policy tests,
o changes to the threat landscape and cybersecurity legal and regulatory requirements and
o changes to the policy on the security of network and information systems and/or topic-specific
policies.
EXAMPLES OF EVIDENCE
...
```
o make sure that personnel are properly trained to handle and manage incidents;
```
```
o identify and consider all external stakeholders (e.g. operators, technology suppliers, free and
```
```
...
o anomaly detection,
o monitor for new vulnerability reports issued for any free and open source software
```
components (21) used by the entity,
```
o data loss prevention,
o forensic investigations support and
o network health monitoring.
```
• Procedures should describe (indicative, non-exhaustive list):
```
o objectives,
o data for collection and relevant tools,
o description of data algorithms and
o mechanisms for notifying the relevant personnel.
```
...
```
o ease of use,
o integration with the existing network and information system, including cross-border operations
and the associated regulatory, security and performance considerations,
o minimisation of manual intervention,
o capability of collecting data from various sources, for example networks, systems and
applications,
o security features offered, for example encryption and access control and
o costs and licencing.
EXAMPLES OF EVIDENCE
• Procedures in place.
• Tools in place.
...
```
```
o establish network traffic patterns;
```
```
o use analytics and machine learning algorithms;
```
o continuously update the automated monitoring tools to adapt to new threats and changes in the
```
environment; and
```
o fine-tune the parameters and thresholds based on the latest data and feedback.
...
```
o relevant outbound and inbound network traffic: traffic volume spikes exceeding 50% of normal
```
traffic in a 10-minute period on a specific port;
```
```
o access to systems and applications: three or more account lockouts within 15 minutes;
```
```
o privileged access: two or more instances of privilege escalation (e.g. normal user to admin) within
```
```
24 hours (23);
```
```
o antivirus: malware is detected on multiple endpoints within a short timeframe;
```
o use of system resources: installations of unauthorised software within a short timeframe.
EXAMPLES OF EVIDENCE
...
o access control,
```
o hashing (section 9.2) and
```
o logging of all access and changes to log files.
```
...
```
o Utilize Network Time Protocol (NTP) servers or Precision Time Protocol (PTP) for accurate and
```
```
reliable time synchronization (24).
```
o Use authenticated NTP to prevent malicious entities from tampering with your time
synchronization.
```
o Configure a central time server within the entity (25). This server should synchronize with a
```
reliable external time source and then distribute the time to all other systems within the network.
o Use multiple time sources to avoid a single point of failure.
```
o Plan how time synchronization is handled across on-premises systems (for example servers in a
```
```
...
```
```
o structured or semi-structured, if possible, instead of unstructured format;
```
o consistent data format in line with the selected tools and well-known standards, for example
```
JavaScript Object Notation and extensible markup language (XML);
```
o log level in line with the classification level of the asset being logged – the entity should assign a
...
```
```
o the standard for the timestamps, for example ISO-8601 (26), RFC 3339 (27) or RFC 9557 (28).
```
```
...
```
```
o source identifier, for example the application or the device relevant to the entry; and
```
o a unique identifier for the entry.
...
```
o if the confidentiality or the integrity or the availability of the network or the information system has
```
been affected;
```
```
o persistence, meaning whether the event is ongoing or not;
```
```
o impact, for example. the number of assets (potentially) affected; and
```
o compliance violation of a regulation or the entity’s policies.
...
o any relevant screenshots, logs or other evidence,
o contact information for follow-up if necessary.
...
```
```
o reasons/motivations for communicating or reporting (business reasons and legal reasons, etc.),
```
o the type of events in scope,
o the required content of communications,
o notifications or reports,
o the channels to be used and
o the roles responsible for communicating, notifying and reporting.
...
such as:
o gathering relevant information and evidence related to the event.
o analysing the potential impact on the entity’s systems, data and operations.
o determining the severity of the incident based on predefined criteria.
```
...
```
o low, medium, high or critical severity;
```
```
o incident types (e.g. malicious software infection or unauthorized access);
```
o regulatory or compliance implications.
...
June 2025, version 1.0
44
o Consider that the root cause of an incident may be challenging to determine during the early
...
```
o minimising data collected, meaning only collecting and analysing logs that fit the purpose – avoid
```
retaining unnecessary personal or sensitive data;
```
```
o anonymising or pseudonymising the collected data, when possible;
```
o applying good security practices where appropriate and relevant, such as access control,
```
encryption, regular audits and monitoring;
```
o applying the data retention policy in alignment with GDPR requirements and regularly purging
```
data that is no longer needed; and
```
o considering data protection and relevant legal and compliance obligations other than the NIS2
Directive.
EXAMPLES OF EVIDENCE
...
of the incident.
o Recognize and address potential conflicts between the following objectives during incident
```
handling:
```
...
operations.
o Where these objectives conflict, establish a clear decision-making process that:
...
accountability.
o Develop incident response playbooks that incorporate decision making and escalation paths for
...
applicable).
```
3.5.4. The relevant entities shall log incident response activities in accordance with the procedures referred to in point
3.2.1. and record evidence.
GUIDANCE
```
• Log incident response information which contains (indicative, non-exhaustive list):
```
```
o time of detection, containment and eradication;
```
```
o when the systems recovered;
```
```
...
```
```
o actions taken during each phase namely, detection, containment and eradication;
```
```
o assessment of the scope and the level of impact of the incident;
```
```
o communications when responding to the incident;
```
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
47
```
o post incident lessons learnt and recommendations; and
```
o whether the CSIRT or the competent authority was notified of the incident according to the NIS2
...
o Where necessary, include management bodies in the tests so that they understand their role
during an incident.
...
( 35 ) For example,
```
• ISO 22301:2019 - Security and resilience — Business continuity management systems — Requirements,• ISO 22313:2020 - Security and resilience — Business continuity management systems — Guidance on the use of ISO 2230 and
...
```
```
o steps followed; and
```
o final recovery time.
```
• Determine the order of recovery based on criteria, including (indicative, non-exhaustive list):
```
```
o the asset classification level;
```
```
o the importance of the service for the entity;
```
```
o dependencies (services or assets that are essential for others are restored first);
```
```
o recovery objectives (Annex to the regulation, point 4.1.3);
```
```
o resource availability; and
```
...
```
o For remote work, employees handling critical operations should:
```
...
```
o failover sites in other regions;
```
```
o backups of data with high criticality to remote locations; and
```
o tested restore procedures with regular validation cycles.
```
...
```
```
o Recovery time objectives (RTOs) to determine the maximum amount of time allowed for the
```
```
...
```
```
o Recovery point objective (RPO) to determine how much data it is acceptable for specific ICT
```
...
```
o Service delivery objective (SDO) to determine the minimum level of performance that needs to
```
...
restored.
```
o Maximum acceptable outage (MAO) or maximum tolerable period of disruption (MTPD) to
```
...
```
o the RTOs, RPOs and SDOs; and
```
o compliance with applicable regulations and legislation.
EXAMPLES OF EVIDENCE
...
```
```
o alternative locations for personnel;
```
```
o disaster recovery locations – hot sites;
```
```
o digital twins;
```
...
```
```
o past incidents; and
```
o results of previous tests.
• Where appropriate, test the disaster recovery plan at an alternate processing site to:
```
o familiarize related personnel with the facility and available resources; and
```
o evaluate the capabilities of the alternate processing site to support operations.
...
```
```
o power failover between power providers and/or power provider to backup (for example
```
```
generators or batteries); and
```
o resilience to maintain service to customers.
...
```
```
o documented results of the continuity of operations test activities; and
```
o records of individual training activities.
...
```
```
o coordination among organisational units;
```
```
...
```
o metrics for measuring the successful implementation of the plan.
...
following:
```
o use checksums or hashing algorithms to verify that the data in your backups matches the original
```
data (section 9.2);
```
```
o implement automated scripts to run these checks regularly, reducing the risk of human error;
```
o schedule regular tests to restore data from backups to ensure they are complete, functional and
```
validated by business users to confirm the accuracy and usability of the restored data;
```
o test various recovery scenarios, including full system restores and individual file recoveries, to
```
ensure all aspects of your backup system are reliable; and
```
o consider using cloud storage solutions for off-site backups, which often include built-in integrity
...
```
```
o network and information systems: one or more of the following (indicative, non-exhaustive list):
```
...
```
```
o assets: one or more of the following (indicative, non-exhaustive list):
```
 shared workspaces,
 backup locations,
 spare equipment,
...
```
```
o personnel: one or more of the following (indicative, non-exhaustive list):
```
 job rotation,
 backup assignments,
```
...
```
```
o multiple communication platforms, for example social media, messaging apps and email; and
```
o multiple methods to power a site, either through multiple electrical providers or with a combination
...
```
```
o prioritisation of resources based on the results of the risk analysis;
```
```
o partial redundancy;
```
```
o diverse backup locations; and
```
o continuous monitoring of the resources where redundancy is necessary.
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
59
EXAMPLES OF EVIDENCE
...
```
As an example:
o Data with high criticality might be checked on a weekly basis.
o Data with moderate and low criticality might be checked on a monthly basis.
o Significant changes should be checked immediately after the change.
...
```
o keep three copies of the data (the original plus two backups),
```
```
o on two different types of storage media (e.g. hard drives, cloud storage),
```
o with one copy stored offsite.
...
```
```
o malware and ransomware scanning of backups before backups are stored;
```
```
o use of immutable backups;
```
o offline storage of backups.
```
...
o backups will be replicated across multiple availability zones or stored in an alternate region to
```
ensure resilience against zone-level failures;
```
```
o will be configured with cross-zone replication to minimise downtime;
```
o all backup data will be encrypted at rest using cloud-native key management systems and
```
encrypted during transfer using TLS 1.2+;
```
o regular backup testing will be performed to validate cross-zone recovery capabilities.
EXAMPLES OF EVIDENCE
...
```
o Be aware that each crisis may be different and further analysis may be required on an ad hoc
basis.
```
o Consider the various aspects (e.g. technical, operational, communication and remediation) of
```
...
```
o the incident poses significant risk to critical assets or operations with high criticality, for example
```
high-severity incidents (e.g. data breaches involving sensitive information);
```
o the incident disrupts business operations significantly, for example prolonged downtime,
```
widespread loss of services or significant impact on customer service;
```
o the breadth of the incident, that is, whether it affects multiple systems, departments or geographic
```
locations, indicating a wider threat;
```
o the potential impact on the entity’s reputation – incidents that could lead to public scrutiny or loss
```
of customer trust should be escalated;
```
o the potential impact of the cybersecurity incident on the confidentiality, integrity, authenticity and
```
availability of data;
```
o the sophistication and motivations of the threat actors involved. Incidents linked to advanced
persistent threats or organised cybercrime may require a higher-level response, beyond the
```
capabilities of the entity;
```
```
o the potential to escalate further (e.g. if vulnerabilities could be exploited again or if malware is
```
```
spreading).
```
...
```
o legal obligations for communication, such as timing of communication, in particular referring to
```
requirements for notification;
```
```
o how information will be disseminated to internal and external stakeholders (employees,
```
```
...
```
o communication channels to be used for each type of stakeholder, considering that:
```
...
indicated;
```
o up-to-date contact information for internal and external stakeholders.
EXAMPLES OF EVIDENCE
...
```
```
o designate a point of contact with the CSIRT;
```
o ensure that the point of contact has sufficient knowledge concerning incidents and threat
intelligence.
o classify incoming information into categories such as incidents, vulnerabilities, threats and
mitigation measures.
o assign priority levels based on severity and potential impact on the entity, if the information is
relevant or applicable
o have the CSIRT contact point review the information for relevance and urgency.
o validate information against internal logs, threat intelligence feeds and existing security policies.
```
o for vulnerabilities and threats, if applicable, collaborate with relevant teams (IT, security,
```
```
operations) to develop a mitigation strategy;
```
o update or create incident response plans based on the nature of the threats or incidents reported
in accordance with point 3.5 of the Annex to the regulation.
o where appropriate, implement the mitigation measures and communicate with the relevant
stakeholders in accordance with point 3.5 of the Annex to the regulation.
o share insights and feedback on incidents and mitigations with the CSIRT on a voluntary basis.
EXAMPLES OF EVIDENCE
...
```
```
o taking into account past crisis situations;
```
o comparing the results of the tests to the objectives defined, for instance the recovery objectives
```
in point 4.1.2 (f) of the Annex to the regulation (e.g. RTOs, RPOs and SDOs); and
```
o using the results of the comparison to update and improve the crisis management procedure.
...
```
```
o procedures for declaring a crisis;
```
```
o activation of the crisis management team;
```
```
o escalation paths;
```
```
o emergency procedures, which describe the actions in the event of a crisis; and
```
o fall back procedures that describe the actions to be taken to protect essential activities or support
```
...
```
```
o how information will be disseminated to stakeholders during a crisis;
```
```
o templates for communication; and
```
o up-to-date contact information for internal and external stakeholders, including employees,
...
```
```
o ICT supplier (including software and hardware supplier),
```
o Manufacturer,
o managed service provider,
o managed security service provider and
o cloud computing provider.
```
...
o the legal jurisdiction of the supplier or service provider, for example whether the supplier is
```
regulated under the NIS2 Directive or the Cyber Resilience Act and in which jurisdiction(s);
```
```
o if available, compliance statements from the supplier in relation to the NIS2 Directive;
```
```
o the corporate ownership of the supplier or service provider;
```
```
o the supplier’s or service provider’s ability to ensure supply (e.g. size, reliance on other suppliers
```
```
or service providers and degree of control over its own supply chain);
```
o the cybersecurity practices of the supplier or service provider, and whether adequate prioritisation
...
```
o notices or advice issued by national authorities on the selection of suppliers or service providers,
```
if available;
```
```
o the sensitivity of the use of the products and services acquired;
```
```
o the supplier or service provider’s history in relation to cybersecurity events and breaches;
```
o the possibility of vendor lock-in, if the supplier or service provider is selected. Parameters to
...
(indicative, non-exhaustive list):
```
o collective bargaining by teaming up with similar size organisations for purchasing products or
```
services in bulk;
```
```
o representation by an association of which the entity is a member of;
```
```
o legal advice for reviewing and negotiating a contract;
```
```
o negotiating specific clauses such as exit, pricing and SLAs (43);
```
```
o suppliers that publicly publish their security, privacy or reliability commitments;
```
```
o providers with standard certifications (e.g. ISO 27001, SOC 2);
```
```
o trusted platforms or marketplaces that vet suppliers (e.g. app stores, SaaS directories, partner
```
```
networks);
```
o contract templates or free legal checklists from small business associations, NGOs or
```
government sites;
```
```
o tools/services that allow easy data export, monthly payments or no lock-in;
```
```
o avoiding long-term commitments or vendor lock-in without clear exit strategies; and
```
```
...
June 2025, version 1.0
69
o a short list of alternatives in case the supplier becomes unavailable.
EXAMPLES OF EVIDENCE
...
```
```
o material changes in their operations;
```
```
o changes in their risk exposure;
```
```
o failure to meet their contractual obligations; and
```
o emergence of new threats or vulnerabilities affecting the provided ICT products or services.
```
...
```
```
o a clear and complete description of ICT products and services;
```
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
71
o service level descriptions, including uptime guarantees or target service levels, response times
```
for service issues and updates to the service level descriptions thereof;
```
```
o locations (regions or countries) where the ICT products are to be produced and ICT services are
```
...
```
o provisions on availability, authenticity, integrity and confidentiality in relation to the protection of
```
...
```
o obligations on the suppliers and service providers, such as retrieval and disposal of the
...
```
o obligations of the supplier or service provider to provide assistance to the entity at no additional
...
```
```
o contacts and reporting lines;
```
```
o the obligation of the supplier or service provider to fully cooperate with the competent authorities;
```
o termination rights and related minimum notice periods for the termination of the contractual
```
arrangements;
```
o notice periods and reporting obligations of the supplier or service provider to the entity, including
...
levels;
```
o the right to audit by the entity or an appointed third party and by the competent authority and the
...
```
o exit strategies, in particular the establishment of a mandatory adequate transition period,
...
```
```
o country-specific information (e.g. threat assessment from national security services), if available;
```
o restrictions or exclusions posed by a relevant national authority, for example equipment with high
```
criticality for the entity or for high-risk suppliers;
```
```
o information stemming from known incidents or cyber threat intelligence; and
```
o the characteristics of each supplier, such as the quality of its security practices, the legal
...
```
o When using FOSS without purchasing a related software product or entering into a service
...
```
o Consider supporting the communities developing and maintaining FOSS and invest in a mutually
...
o Consider steps to mitigate the identified risks to the security of network and information systems
...
```
o Engage with relevant open source communities to stay informed about significant updates,
...
```
o results from risk assessments of the FOSS,
o dependency monitoring tools,
o documentation.
...
(indicative, non-exhaustive list):
```
o sensitivity of assets purchased,
o volume of assets purchased,
o availability requirements,
o applicable regulations,
o inherent risk and mitigated risk.
• Update and review classifications annually or when significant changes occur. Examples of categories
may be:
```
o critical – those with a significant impact on the entity’s operations;
```
o strategic – high-value partners who contribute to information assets, for example cloud providers,
```
data analytic providers, software developers and telecommunication providers;
```
o routine – those with minimal impact on the entity.
EXAMPLES OF EVIDENCE
...
```
( 48 ) See section 5.1 (Tips), for additional guidance on the use of free and open source software supply chain.( 49 ) In addition, to those mentioned in the mapping table at the end of this section, consider the following:
```
...
(indicative, non-exhaustive list):
```
o the potential risks that could arise from acquiring the specific ICT service, system or product. This
might involve penetration testing to identify threats, vulnerabilities and the potential impact on the
```
entity’s operations;
```
o potential security tools that already need to be in place, for example a firewall, an intrusion
```
detection system, a SIEM or a EDR/XDR;
```
o a specific security mechanism that might need to be in place, such as a specific encryption
```
algorithm or a particular access control mechanism (e.g. MFA);
```
o cybersecurity standards for the ICT service, system or product that the entity needs to comply
```
with;
```
o where appropriate, the required assurance level of the ICT product, system or service and the
...
```
constructively engage with project maintainers in addressing Identified security requirements and introducing mitigations and/or improvements. Theseimprovements should be shared with the open source project under their license and terms.
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
78
...
```
o ensure continuous alerting, patching and mitigation proposals if vulnerabilities in the system or the
```
product are discovered;
```
o clarify the supplier’s or service provider’s liability in the event of cyber-attacks or incidents relevant to
```
the service, system or product; and
```
```
o consider cybersecurity during project implementation and before handover including (indicative, non-
```
```
exhaustive list):
```
...
```
o masking or pseudonymization on fields like names, emails, IDs;
```
```
o deletion or redaction of identifiers (e.g. personal IDs, birth dates,
```
```
ZIP codes); and
```
o non-irreversible or one-way anonymisation.
...
```
o documentation for each phase of the life cycle,
o process and workflow diagrams,
o audit and testing reports,
o version control,
...
reviews.
o Specific sections in the documents highlighting what changes were made and the rationale behind
them.
...
```
o documentation of the process used for example STRIDE or DREAD,
o data flow diagrams, and
o meeting minutes.
...
exhaustive list):
```
o processing and handling of information,
o backup,
o scheduling requirements, including interdependencies with other systems,
o handling errors or other exceptional conditions,
...
June 2025, version 1.0
83
o cryptographic mechanisms and settings, and
o audit trail and system log information.
```
...
```
o registry settings,
o account, file and directory permission settings, and
o settings for functions, ports, protocols, services and remote connections.
...
o Where appropriate, implement a configuration management database to catalogue and classify
```
...
```
```
o essential capabilities of operation;
```
```
o restricted use of functions;
```
```
o security by default;
```
o ports, protocols and/or services allowed.
...
```
o roles, responsibilities and configuration management processes and procedures;
```
```
o a process for identifying CIs throughout the system development life cycle; and
```
o a process for managing the configuration of the CIs throughout their life cycle.
...
o risk assessment,
o criteria for categorisation and prioritisation of changes:
 associated requirements for the type and scope of the tests to be carried out and
```
 the approvals to be obtained;
```
```
o requirements for performing rollbacks; and
```
o documentation of the changes and approval of changes.
...
```
```
o management commitment, if applicable;
```
```
o coordination among different organisational units; and
```
o compliance.
...
```
```
o verifying that no information related to the entity is contained in the equipment;
```
```
...
```
o obtaining an exemption from authorised personnel or roles explicitly authorising removal of the
...
```
```
o approved parties (internal or third);
```
```
o confidentiality levels for assessment; and
```
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
91
o test results and the objectives of security assessments and tests.
...
June 2025, version 1.0
92
o Blue/green allows the patches to be applied first in an isolated environment identical to the
production environment, and then in the production environment. This ensures zero downtime
and an immediate rollback option.
```
o Rolling (59) deployment allows gradual updates of parts of the production environment, one set
```
of servers or instances at a time, rather than deploying a patch all at once. It is ideal for large and
distributed environments.
o Feature toggles allow new features or patches to be deployed to production but keep them
...
o Shadow deployment allows new code or patches to be deployed directly to production but it
...
o Hotfix deployments are used for critical patches that need to be applied immediately to address
severe issues.
...
```
o vendor documentation on the patch:
...
```
```
o severity rating - patches that address critical vulnerabilities are more likely to apply; and
```
o security blogs, forums and mailing lists for any known issues or incompatibilities introduced by
the patch.
...
```
```
o standards such as Start transport layer security (STARTTLS), DNS-based authentication of
```
```
...
```
o internal spam/scam/virus filtering and
o URL rewriting, URL scanning and URL detonation in a sandbox.
```
• Consider DNS security good practices (indicative, non-exhaustive list):
```
```
o deploying DNS security extensions (DNS SEC) (64),
```
```
...
```
implement zero trust are (indicative, non-exhaustive list):- Identity and access management supported by MFA (11.6.1), role based access control (11.1.3) and just in time access;
```
- Device security supported by a policy which allows only known devices and EDR (6.9.2);- Network microsegmentation supported by logical segmentation (6.8), Next Generation Firewalls and a policy which prevents lateral movement;
...
```
grants access to resources based on the combination of identity/device trust/context;- Application and data protection supported by cloud access security brokers (CASBs), data loss prevention (DLP) and encryption as well as
```
```
...
June 2025, version 1.0
96
o deploying protective DNS, wherever technically feasible, to provide additional network-wide
security capabilities,
o encrypting DNS traffic, both internal and external, wherever feasible,
o deploying dedicated DNS servers to reduce attack surface and
o following all technical guidance on ensuring that DNS deployments and the DNS protocol are as
...
```
```
o continuously monitor the networks for real time threats;
```
```
o perform scans for new vulnerabilities weekly;
```
```
o review and possibly update the rules of the firewall and other tools monthly; and
```
o thoroughly assess the entire network annually.
...
```
o publicly accessible systems;
```
```
o internal networks;
```
```
o OOB connections; and
```
o assets with high criticality.
```
...
o in separate racks in the same room,
o in separate rooms for the components with high criticality and
o more significant geographical separation of the components with high criticality.
```
...
```
```
o network-based malicious code analysis;
```
```
o virtualisation systems; and
```
o encrypted tunnels.
...
```
o be active all the time;
```
o perform periodic scans of the system regularly and real-time scans of files from external sources
```
as the files are downloaded, opened or executed;
```
```
o generate notifications when suspected malicious and unauthorised software is detected;
```
```
o disinfect and quarantine infected files; and
```
o restore system settings and ensure that critical settings cannot be disabled or restricted.
...
```
```
o implementing rules and measures that prevent or detect the use of unauthorised software;
```
```
o implementing measures that prevent or detect the use of known or suspected malicious websites;
```
```
o reducing vulnerabilities that can be exploited by malicious software;
```
```
o controlling the running of applications on user workstations or user end devices (including
```
```
smartphones or tablets);
```
o employing web application filters to reduce exposure to malicious content.
...
```
```
o phishing filters; and
```
o machine learning filters.
EXAMPLES OF EVIDENCE
• Documented alternative countermeasures such as:
```
o securing all physical and logical data interfaces;
```
o network isolation and implementation of segmented network security zones that limit the impact of a
```
malicious software incident;
```
o comprehensive system-hardening measures to minimise the risk of malicious software incidents.
o logs confirming that administrative privileges are controlled and monitored.
...
and evaluation.
```
• Information Technology Infrastructure Library (ITIL).(73 ) For example, NIST SP 800-53A available at: https://csrc.nist.gov/pubs/sp/800/53/a/r5/final.
```
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
108
o self-assessment,
o benchmarking against a measure’s checklist or a standard,
o vulnerability assessment,
```
o penetration testing (e.g. internal, external, red/blue team),
```
o secure code review,
```
o audit (e.g. internal, external, compliance) and
```
o performance monitoring.
• The assessment service can be provided by an external entity or by specially authorised employees of the
entity.
o In the case of external entities, confidentiality and non-disclosure terms should be included in the
contract.
o Internal employees should be suitably trained, based on the skills required for the objective of
...
```
```
o the cost of implementation and maintenance, for example capital expenditure (CAPEX) /
```
```
operational expenditure (OPEX);
```
```
o the number of employees who have attended cybersecurity trainings (75);
```
```
o the number of vulnerabilities detected;
```
```
o time to remediation;
```
```
o the number of incidents;
```
```
o the number of incidents related to a change;
```
```
o incident response times; and
```
```
o number of non-compliances (consider the elements in point 2.2 of the Annex to the regulation
```
```
...
o continuously monitor and measure the effectiveness of mitigating measures designed to address
```
real-time threats (e.g. firewalls, IDS/IPS);
```
o monitor and measure the effectiveness of security measures related to the threat landscape
```
biannually (e.g. vulnerability management and incident response plans);
```
```
o assess the overall effectiveness of all measures annually;
```
```
o assess the effectiveness of measures related to a specific incident following that incident; and
```
```
( 74 ) ENISA’s cybersecurity investment reports offer a good reference for measuring effectiveness of measures, for example the 2023 NIS investmentsreport, https://www.enisa.europa.eu/publications/nis-investments-2023.
```
```
...
June 2025, version 1.0
109
o assess the effectiveness of measures related to a specific system or one of its components
...
```
```
o changes to the environment of operation; and
```
```
o trends related to threats and vulnerabilities (take into account root cause analysis) (76).
```
```
...
```
( 76 ) More information on information security incident root cause analysis can be found in Forum of Incident Response and Security Teams, ‘FIRSTCSIRT services framework’, Version 2.1, ‘6.2.4 Function: Information security incident root cause analysis’,
```
```
https://www.first.org/standards/frameworks/csirts/csirt_services_framework_v2.1#6-Service-Area-Information-Security-Incident-Management
```
...
```
o use various formats, such as workshops, webinars and e-learning modules;
```
```
o use multiple communication channels (emails, newsletters and intranet) to keep employees
```
...
```
o clear desk and screen policy,
o use of relevant strong authentication means and methods, multi factor passwords etc,
o event reporting,
o safe email use and web browsing,
o protection from phishing and social engineering,
o secure use of mobile devices,
o secure use of the entity’s connected-vehicles,
o secure connection practices,
```
...
112
o backup practices,
o zero-trust concept,
o software updates,
o secure device configuration,
o network segmentation,
o secure teleworking practices.
```
• Include the following in the programme (indicative, non-exhaustive list).
```
o Train personnel on the policy on the security of network and information systems.
o Train personnel to recognize social engineering attacks, such as phishing, pre-texting and
tailgating.
o Train personnel to be aware of causes of unintentional data exposure. Example topics include
...
o Train personnel on the dangers of connecting to and transmitting data via insecure networks for
...
o Train personnel in understanding malicious and unauthorised software, on the importance of
...
```
```
( 79 ) Communication from the Commission to the European Parliament and the Council – Closing the cybersecurity talent gap to boost the EU’scompetitiveness, growth and resilience ('The Cybersecurity Skills Academy'), COM(2023) 207 final, https://eur-lex.europa.eu/legal-
```
...
```
( 80 ) https://www.enisa.europa.eu/topics/skills-and-competences/skills-development/european-cybersecurity-skills-framework-ecsf.( 81 ) https://www.enisa.europa.eu/topics/skills-and-competences/skills-development/european-cybersecurity-skills-framework-ecsf.
```
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
114
...
```
o Train personnel in authentication best practices, such as MFA, password creation and credential
management.
o Train personnel in how to identify and properly store, transfer, archive and destroy sensitive data.
o Train personnel to recognise a potential incident, such as unusual email attachments, unexpected
system behaviour and suspicious network traffic.
o Train personnel in how to report events promptly and accurately, including the use of designated
communication channels.
o Train personnel to understand how to verify and report out-of-date software or any failures in
...
o Train relevant personnel in crisis management and business continuity procedures. Incorporate
```
simulated events ( 82 ) into crisis management training to facilitate an effective response by
```
personnel in crisis situations.
```
o Train relevant personnel (e.g. system administrators and software developers) in the secure
```
...
```
o ensure secure disposal of the data after its lawful use.
```
...
```
```
o scanning network traffic;
```
```
o using secure online (e.g. client-side cloud encryption) and offline storage; and
```
o removing sensitive data from storage media.
...
```
o generate keys for different cryptographic systems and different applications;
```
```
o generate and obtaining public key certificates;
```
```
o distribute keys to intended users; and
```
o deal with compromised keys.
...
```
```
o flexibility in algorithm selection;
```
```
...
June 2025, version 1.0
119
o modular design of the architecture whereby cryptographic components can be changed or updated
```
independently without affecting the entire system;
```
```
o regular updates and patching;
```
o compliance with the legislative frameworks and governance of the use of cryptography within the
```
entity’s networks and information systems;
```
o future-proofing by considering quantum cryptographic algorithms.
...
o executives and senior management,
o roles with access to sensitive information,
o roles with financial responsibilities,
o roles involving procurement and vendor management,
o roles that grant access to physical assets or are responsible for physical security.
```
...
```
o the nature (who, what, when, how) and gravity of the violation and its consequences;
```
```
o whether the offence was intentional (malicious) or unintentional (accidental);
```
```
o whether this is a first or repeated offence;
```
o whether or not the employee who committed the violation was properly trained.
...
o Access control rules can be implemented in different granularities, ranging from covering whole
...
o Use business requirements and risk assessment results to define which access control rules are
...
```
o consistency between access rights and asset classification;
```
```
o consistency between access rights and physical perimeter security needs and requirements;
```
o consideration of all types of available connections in distributed environments so entities are only
provided with access to associated assets, including networks and network services, that they
```
are authorized to use;
```
o consideration of how elements or factors relevant to dynamic access control can be reflected.
...
o need-to-know: an entity is only granted access to the information that it requires to perform its
...
profiles);
```
o need-to-use: an entity is only assigned access to information technology infrastructure where a
clear need is present.
• Consider the following when specifying access control rules:
```
o establishing rules based on the premise of least privilege (‘everything is generally forbidden
```
```
...
June 2025, version 1.0
132
o changes in user permissions that are initiated automatically by the network and information
```
system and those initiated by a system administrator;
```
o when to define and regularly review the approval.
```
...
```
o cover granting access rights to assets upon the new hire or role change of a user;
```
o obtain authorization from the owner of the asset, separate approval for access rights by
```
management bodies can also be appropriate;
```
```
o ensure that access rights are activated (e.g. by service providers) only after authorization
```
```
procedures are successfully completed;
```
```
o consider the business requirements and the entity’s access control policy;
```
o consider segregation of duties, including segregating the roles of approval and implementation
```
...
June 2025, version 1.0
133
o verify that the level of access granted is in accordance with the access control policy and is
```
consistent with other information security requirements such as segregation of duties;
```
o consider giving temporary access rights for a time and revoking them at the expiry date, in
...
o timely disable accounts upon the termination, rights revocation or role change of a user, as
```
...
```
```
o modify the access rights of users who have changed roles or jobs;
```
o remove or adjust access rights, which can be done by removing, revoking or replacing keys,
authentication information, identification cards or subscriptions.
```
o where feasible, enable different components or services to share information (signals) about
```
...
```
o users’ access rights after termination or change of employment;
```
o authorisations for privileged access rights.
...
136
```
o Consider the frequency of system administration operations: daily tasks (e.g. backups and email
```
```
...
o The inventory should include both user and privileged or system administrator identities. The
...
o The inventory should also include all service identities and have these identity records as a
...
```
o confirming the business requirements for an identity to be established;
```
```
o verifying the identity of an entity before allocating them a logical identity;
```
```
o establishing an identity;
```
o configuring and activating the identity, which also includes configuration and initial setup of
```
related authentication services; and
```
o providing or revoking the identity’s specific access rights, based on appropriate authorization or
```
...
```
```
o Least privilege concept enforcement;
```
```
o Time-bound or just-in-time access;
```
```
...
June 2025, version 1.0
140
o Checking out the shared credentials through a tool which logs who accessed the identity and
when. If check-out is not possible, consider using privileged session recording or proxy-based
```
access auditing;
```
```
o Session attribution techniques to link shared identity use back to individual users;
```
```
o Logging;
```
```
o Credential vaulting with auto-rotation;
```
```
o Prohibition of storing shared credentials in personal files, emails or messaging platforms;
```
o Network segmentation so that shared account use is limited to isolated network segments or
```
virtual environments; and
```
o Mandatory training on acceptable use and accountability.
...
```
o token-based authentication, such as a one-time passcode (OTP),
```
o smart cards,
o Fast Identity Online 2 security keys,
...
o passwords or PINs generated automatically during enrolment processes as temporary secret
```
...
```
o procedures are established to verify the identity of a user prior to providing new, replacement or
```
temporary authentication information;
```
o authentication information, including temporary authentication information, is transmitted to users
```
...
```
```
o users acknowledge receipt of authentication information;
```
o default authentication information as predefined or provided by suppliers is changed immediately
```
following installation of systems or software;
```
o records of significant events concerning allocation and management of authentication information
```
...
o allow users to select and change their own passwords and include a confirmation procedure to
```
address input errors;
```
```
o enforce strong passwords (96);
```
```
o force users to change their passwords at first login, if relevant;
```
o enforce password changes as necessary, for example after a security incident or upon
...
```
o prevent the use of commonly used passwords, and compromised combinations of usernames
```
and password from hacked systems;
```
```
o not display passwords on the screen when they are being entered; and
```
o store and transmit passwords in protected form.
...
```
o a domain, in accordance with Fast Identity Online (FIDO) and W3C
```
```
WebAuthn standards;
```
...
June 2025, version 1.0
143
o a trust provider, following public key infrastructure and International
Telecommunication Union X.509 standards.
o ‘Medium’ MFA, for example:
 push notification, number matching or application based.
o ‘Last resort’ MFA, for example:
...
o Secret authentication information, such as passwords, is kept confidential. Personal secret
...
authorised persons.
o Affected or compromised authentication information is changed immediately upon notification of or
any other indication of a compromise.
o When passwords are used as authentication information, strong passwords according to best
...
o The same credentials are not used across different network and information systems.
o The obligation to follow these rules is also included in the terms and conditions of employment.
EXAMPLES OF EVIDENCE
...
```
o test-message-based OTP: simple but less secure due to risks such as SIM swapping;
```
```
o authenticator apps: generate time-based OTPs;
```
```
o push notifications: send an approval request to a user’s device;
```
```
o hardware tokens: for example physical devices generating OTPs, smart cards;
```
```
o passkeys;
```
...
```
o ease of integration: ensure the MFA solution integrates well with the existing systems;
```
```
o user experience: aim for a balance between security and user convenience;
```
```
o scalability: choose a solution that can grow with the entity;
```
o support and reliability: ensure the provider offers robust support and high reliability.
...
```
```
o sharing or restricting information;
```
```
o protecting the integrity and authenticity of information;
```
```
o ensuring availability (100); and
```
o complying with legal requirements concerning the confidentiality, integrity or availability of the
information.
```
...
```
```
o internal - accessible only to members of the entity;
```
...
```
o changes in the value, sensitivity and criticality of the assets throughout their life cycle.
EXAMPLES OF EVIDENCE
...
```
o create user manuals and training materials on the correct and secure use of assets;
```
```
o establish guidelines for secure storage, taking into account backup management (section 4.2);
```
o define protocols for secure transfer, including consider secure migration processes, when
```
transferring data to a cloud service;
```
o outline methods for data wiping and physical destruction, ensuring complete and irretrievable
deletion.
...
```
( 103 ) Vehicles that can store data locally on the vehicle and/or share data externally by means of telematics are included.( 104 ) Data that moves between systems, users, or devices, often temporarily or for processing purposes.
```
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
151
...
```
o definitions and scope of removable media,
o authorization requirements,
o usage guidelines,
```
( 105 ) Including ‘bring your own device’, if personal devices are used to store corporate data. Vehicles that can store data locally on a vehicle and/orshare data externally by means of telematics are also included.
```
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
152
```
o measures for control and protection of removable media while in storage and in transit;,
```
o techniques to protect information on removable storage media and
o incident response procedures for lost or compromised media.
...
o asset unique ID,
```
o asset type, for example software including virtual machines (version), hardware (operating
```
```
...
```
o asset owner and contact information,
o operational unit responsible for the asset, either internal department name or external provider
```
name (106),
```
o asset description,
o asset location,
o date of asset’s last update/patch,
o asset classification consistent with the risk assessment,
o type of information and its classification processed in the basset,
o asset end of life, where applicable,
o relation to other assets and
...
```
o user endpoint devices, e.g. computers, tablets and phones etc and/or portable storage devices,
...
```
```
o authentication hardware (e.g. access cards, mechanical keys, physical tokens and smart cards);
```
o physical copies of information.
EXAMPLES OF EVIDENCE
...
```
```
o ensure that any credentials associated with the assets are revoked or disabled;
```
```
o isolate the assets by placing them in a separate network segment;
```
```
o use access control lists to restrict access to and from the isolated assets;
```
```
o ensure that only authorised personnel can interact with these assets;
```
```
o configure firewalls to block any traffic to and from the isolated assets;
```
```
o physically or logically disable the network interfaces of the assets;
```
o continuously monitor the isolated assets and log any access attempts.
EXAMPLES OF EVIDENCE
...
```
```
o power supply – electricity to keep systems running;
```
```
o water for cooling and other operational needs;
```
```
o gas for heating or backup power generation;
```
```
o HVAC to maintain optimal operating conditions;
```
o telecommunications – internet and network connectivity.
...
```
```
o active/passive cooling;
```
```
o automatic restart after power interruption;
```
```
o battery backup power;
```
```
o diesel generators;
```
...
```
```
o uninterruptable power supply, hot standby power generators;
```
```
o sufficient fuel delivery SLA;
```
```
...
```
```
o spare parts for components of network and information systems; and
```
o power backup systems.
EXAMPLES OF EVIDENCE
...
```
```
o Include in the assessment the (physical) locations of the entity’s facilities.
```
...
o identification and evaluation of physical locations, such as data centres, offices and server rooms,
o analysis of potential physical and environmental threats and
o mapping of critical assets to physical locations.
...
```
```
o network and information systems in scope;
```
```
...
o coordination among organisational units;
```
o compliance with national and EU law, including personal data protection.
```
...
```
```
o fire, flood or natural events (e.g. earthquakes, storms),
```
o public disturbances or unauthorized access, theft or vandalism,
```
o hazardous material incidents (e.g. chemical spills),
```
```
o long-term environmental changes (e.g. climate trends, air quality).
```
```
• Consider measures against physical and environmental threats such as (indicative, non-exhaustive list):
```
```
o physical access control measures (e.g. IDs, badges, logs; visitor management system and
```
```
physical barriers);
```
```
o surveillance systems (e.g. CCTV, entry points, exits, locking mechanisms and security
```
```
personnel);
```
```
o climate control (e.g. temperature and humidity controls and HVAC systems);
```
```
o fire prevention and response measures (e.g. fire alarms, smoke detectors, sprinkler systems and
```
```
fire extinguishers);
```
...
```
o Increased security personnel, advanced biometric access controls and lockdown procedures.
o Enhanced monitoring systems, redundant power supplies and advanced environmental sensors.
EXAMPLES OF EVIDENCE
...
security measures.
o Conduct both announced and unannounced tests
EXAMPLES OF EVIDENCE
...
o Physical access control measures designed to protect the entity as a whole will also protect
individual assets.
o Consider introducing further specific access control measures for specific assets or facilities.
```
• Consider physical security measures (indicative, non-exhaustive list):
```
o physical access controls such as key cards, biometric scanners, locks and security personnel to
restrict access to high-criticality areas,
o electronic control of entry, with an audit trail,
o segmentation of spaces or creation of zones according to authorization levels and their contents,
o CCTV cameras and monitoring systems to continuously observe sensitive areas,
o fencing, barriers and security patrols for securing physical perimeters,
o guards and/or alarms to monitor every physical access point to the facility where the information
...
(section 11.2).
```
o Identify a designated official within the entity to review and approve the list of personnel with
authorized physical access.
o Maintain a list of personnel with authorized access to facilities, and their authorization levels.
EXAMPLES OF EVIDENCE
...
Belgium
Belgium has completed the incorporation of the NIS2 Directive into national legislation. In that legislation, a special role
```
is reserved for the CyberFundamentals framework (CyFun®, www.cyfun.eu).
```
Finland
```
Traficom (the Finnish Transport and Communications Agency) has issued a national recommendation on
```
cybersecurity risk-management measures for supervisory authorities. The Cybermeter/Kybermittari is a maturity
```
model developed by the National Cyber Security Centre (NCSC-FI) and based on the Cybersecurity Capability
```
Maturity Model and the NIST cybersecurity framework. They are the two instrumental documents for the national
regulatory framework. The recommended cybersecurity risk-management measures are mapped to the
Cybermeter/Kybermittari’s objectives and practices that enable organisations to self-assess their cybersecurity
capabilities and optimise their security investments. Voluntarily sharing quantitative self-assessment data to NCSC-FI
enables the creation of benchmarking data and improves situational awareness of NCSC-FI.
Germany
In Germany, there is an advisory on what requirements should be seen as state of the art. The document is available
at https://www.bsi.bund.de/dok/408936.
For information on BSI standards and certification, see https://www.bsi.bund.de/dok/it-grundschutz-en.
Greece
Greece has incorporated the NIS2 Directive into national legislation with Law 5160/2024. Furthermore, regarding
cybersecurity measures, the Greek national framework consists of the following:
• ministerial decision 1689/2025 ‘National framework of cybersecurity requirements for essential and
```
important entities’;
```
• the Cybersecurity Handbook, available at https://cyber.gov.gr/wp-content/uploads/2025/04/Cybersecurity-
```
Handbook-English-version-1.pdf;
```
• the self-assessment tool, available at https://cyber.gov.gr/wp-content/uploads/2025/03/Cybersecurity-
Self-Assessment-Tool-English-version-1.zip.
The Cybersecurity Handbook and the self-assessment tool are based on globally accepted international standards and
```
guidelines (Center for Internet Security Controls, ISO 27002, NIST 800-53, Open Worldwide Application Security
```
```
Project, etc.) and will be dynamically modified to follow changes in standards, the current threat landscape and the legal
```
and regulatory framework.
Spain
Royal Decree 311/2022, of 3 May 2022, regulates the national security framework, which is a legal regulation that is
mandatory for all entities in the Spanish public sector to comply with and is mandatory to apply to the information
systems used by private companies to provide services to the abovementioned public entities.
The full text of the framework is available at https://ens.ccn.cni.es/es/docman/documentos-publicos/39-boe-a-2022-
7191-national-security-framework-ens/file.
ANNEX II
GLOSSARY
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
...
ANNEX II GLOSSARY
```
Asset: anything that is of value to the entity, including information. Overall, the assets of a network and information
```
system are personnel, processes, information, software and hardware.
```
Crisis: an abnormal or extraordinary event or situation that threatens an organisation or community and requires a
```
```
strategic, adaptive and timely response to preserve its viability and integrity (110).
```
```
Critical: one of the classification levels assigned to the entity’s assets following an assessment of their criticality,
```
```
including the asset classification (Annex to the regulation, point 12.1).
```
```
Cyber hygiene practices (111):
```
• Cyber hygiene practices for essential and important entities are a common baseline set of network and
information security practices, which are already covered by the technical and methodological requirements
of the cybersecurity risk-management measures outlined in the Annex to the regulation. Therefore, no
additional guidance on cyber hygiene practices for essential and important entities is deemed necessary.
• Cyber hygiene practices relating to users are a set of routine, proactive practices and behaviours that the
entity’s users follow to maintain the network and information security of the entity’s systems. These practices
are explicitly mentioned in the guidance of section 8.1.
Direct suppliers and service providers: any entity providing ICT products, ICT systems or ICT services that an
essential or important entity relies on. When this guidance refers to direct suppliers and service providers, it includes
their personnel.
```
Entity: one of the relevant entities in the scope of Commission Implementing Regulation (EU) 2024/2690. In other
```
standards or good practice guides, the term ‘organisation’, ‘enterprise’ or ‘business’ may be used.
```
Event: in reference to an information security event, an identified occurrence indicating a possible information security
```
```
breach or failure of controls (112).
```
```
...
```
recover from, an incident (NIS2 Directive Article 6(8)).
```
```
Incident: an event compromising the availability, authenticity, integrity or confidentiality of stored, transmitted or
```
```
processed data or of the services offered by, or accessible through, network and information systems (NIS2 Directive
```
```
Article 6(6)).
```
```
Information: data in context. In the text, we primarily use ‘information’, unless we refer specifically to data (‘data breach’
```
```
etc.).
```
Management bodies: the highest-level leadership within essential and important entities as in the context of Article 20
of the NIS2 Directive.
```
Measure: a cybersecurity risk-management measure as referred to in the NIS2 Directive. The word is used similarly to
```
```
‘control’, which denotes a measure that modifies risk (113). In addition, the terms ‘measure’ and ‘protection measure’ are
```
used interchangeably.
```
( 110 ) ISO 22361: 2022, 3.2.
```
```
( 111 ) Recital 20 of the regulation addresses cyber hygiene practices for two target groups under Directive 2022/2555: essential and important entities;
```
and their users.
```
( 112 ) ISO/IEC 27002:2022, 3.1.14.
```
```
( 113 ) ISO/IEC 27002:2022, 3.1.8.
```
TECHNICAL IMPLEMENTATION GUIDANCE
June 2025, version 1.0
168
```
Personnel: persons doing work under the entity’s direction ( 114 ). The concept of personnel includes the entity’s
```
members, such as the governing body, management bodies, employees, temporary staff, contractors and volunteers.
In this document it is used interchangeably with the term ‘employees’.
```
Policy: intentions and direction of an organisation, as formally expressed by its management bodies.
```
Privileged access: the necessary permissions granted to specific users of the network and the information system of
the entity, in order for them to perform tasks that regular users cannot.
```
Procedure: a specified way to carry out an activity or a process (115). The entity can document its needs for more
```
detailed information in a way that is efficient for it beyond the policy. These are mainly procedures and processes.
```
Process: an activity that transforms input into output.
```
```
Rule: accepted principle or instruction that states the entity’s expectations of what is required to be done and what is
```
```
allowed or not allowed. Rules can be formally expressed in topic-specific policies and in other types of documents (116).
```
Significant incident: an incident that meets the criteria of Article 3 of the regulation.
Suspicious event: an event that appears unusual or a previously unknown situation that might be a potential security
threat. To make clearer the difference between an event and a suspicious event, consider an example where a legitimate
user fails to log in once due to a typing error. This is an event. However, a situation where a user fails to log in after five
attempts might be considered a suspicious event.
Topic-specific policy: a policy on a specific subject or topic, as formally expressed by the management bodies relevant
to the topic.
```
User: all legal and natural persons that have access to the entity’s network and information systems (recital 10 of the
```
```
regulation).
```
```
( 114 ) ISO/IEC 27002:2022, 3.1.20.
```
```
( 115 ) ISO 30000:2009, 3.12.
```
```
( 116 ) ISO/IEC 27002:2022, 3.1.32.
```
ABOUT ENISA
